...
def main():
    ...
    with st.spinner("Finding car image..."):
        try:
            results = DDGS().images(f"{year} {make} {model_name} car", max_results=1)
            if results and len(results) > 0:
                img_url = results[0].get('image')
                if img_url:
                    st.image(img_url, caption=f"{year} {make} {model_name}", width=500)
        except Exception:
            pass # Keep it clean and fail silently if no internet connection exists
